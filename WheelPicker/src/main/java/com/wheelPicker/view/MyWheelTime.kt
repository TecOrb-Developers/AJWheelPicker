...
class MyWheelTime {
    ...
    fun setPicker(year: Int, month: Int, day: Int, h: Int, m: Int, s: Int, ampm: Int) {
        ...
        val month_names_little = arrayOf("April", "June", "September", "November")
        val list_big = Arrays.asList(*month_names_big)
        val list_little = Arrays.asList(*month_names_little)
        ...
        val monthNames = ArrayList(Arrays.asList(*months))

        wv_month!!.adapter = ArrayWheelAdapter(monthNames)
        ...
        if (list_big.contains(monthName)) {
            wv_day!!.adapter = NumericWheelAdapter(1, 31)
        } else if (list_little.contains(monthName)) {
        ...
        val wheelListener_year = object :OnItemSelectedListener{
            override fun onItemSelected(index: Int) {
                ...
                // 判断大小月及是否闰年,用来确定"日"的数据
                var maxItem = 30
                if (list_big.contains(wv_month!!.currentItem.toString())) {
                    wv_day!!.adapter = NumericWheelAdapter(1, 31)
                    maxItem = 31
                } else if (list_little.contains(wv_month!!.currentItem.toString())) {
                    wv_day!!.adapter = NumericWheelAdapter(1, 30)
                    maxItem = 30
                } else {
       ...
       /* val wheelListener_year = OnItemSelectedListener { index ->
        ...
        // 添加"月"监听
        val wheelListener_month = object:OnItemSelectedListener{
            override fun onItemSelected(index: Int) {
                ...
                if (list_big.contains(months[index])) {
                    ...
                    if ((wv_year!!.currentItem + startYear) % 4 == 0 && (wv_year!!.currentItem + startYear) % 100 != 0
